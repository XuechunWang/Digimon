...
def spell_checked_essay(cor_list):
    cor_essay_ls = []
    for c in cor_list:
        cor_sentence = c.get('result')
        cor_essay_ls.append(cor_sentence)
    cor_essay = ' '.join(cor_essay_ls)
    return cor_essay
...
def question(request, question_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            K.clear_session()
            sentences = essay_to_sentences(content)
            corrections = correct_sentences(sentences)
            feedback = show_feedback(corrections)

            show_feedback(corrections)
